...
router.use(async (req, res, next) => {
  ...
  try {
    ...
    if (!match || !/^\d+$/.test(match[2])) return next();
    const table = match[1] === 'leads' ? 'crm_leads' : 'crm_followups';
    const condition = match[1] === 'leads'
      ? `id=$1 AND LOWER(COALESCE(assigned,'')) IN (LOWER($2),LOWER($3),$4)`
      : `id=$1 AND (assigned_user_id=$4 OR LOWER(COALESCE(assigned,'')) IN (LOWER($2),LOWER($3),$5))`;
...
ensureIndustryDataColumns().catch((err) => console.error('industry data setup failed:', err.message));
...
ensureExportSalesTables().catch((err) => console.error('export sales table setup failed:', err.message));
...
const resolveDigestRecipients = async (followups = []) => {
  ...
  // Keep every recognised salesperson for the admin report.  Email is only a
  // delivery requirement, not an assignment requirement.
  const salespeople = [...configuredSalespeople, ...salesUsers].reduce((people, person) => {
    const existing = people.find((item) => normalize(item.name) === normalize(person.name));
    if (existing) {
...
router.get('/leads', async (req, res) => {
  try {
    ...
    const { rows } = req.crmIsAdmin
      ...
      : await pool.query(`SELECT * FROM crm_leads WHERE LOWER(COALESCE(assigned,'')) IN (LOWER($1),LOWER($2),$3) ORDER BY created_at DESC`, [req.crmUser.full_name || '', req.crmUser.email, String(req.crmUser.id)]);
...
router.get('/followups', async (req, res) => {
  try {
    ...
    const { rows } = req.crmIsAdmin
      ...
      : await pool.query(`SELECT * FROM crm_followups WHERE status <> 'Completed' AND (assigned_user_id=$1 OR LOWER(COALESCE(assigned,'')) IN (LOWER($2),LOWER($3),$4)) ORDER BY created_at DESC`, [req.crmUser.id, req.crmUser.full_name || '', req.crmUser.email, String(req.crmUser.id)]);
