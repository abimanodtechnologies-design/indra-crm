...
const PRODUCT_CATEGORIES = ["Packaging Machine", "PP Strap Roll", "Line Automation"];
...
const MachineRequirementSection = ({ formData, setFormData, userOptions = DEFAULT_USER_OPTIONS }) => {
  ...
  return (
    <div style={{ marginBottom: 24, padding: 18, border: `1px solid ${COLORS.border}`, borderRadius: 10, background: "#fbfdfb" }}>
      <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 16, color: COLORS.primary }}>Machine Requirement</div>
      <div style={grid}>
        <FormField label="Product Category"><select value={formData.productCategory || "Packaging Machine"} onChange={e => update("productCategory", e.target.value)} style={inputStyle}>{PRODUCT_CATEGORIES.map(v => <option key={v}>{v}</option>)}</select></FormField>
        <FormField label="Machine Type"><select value={formData.machineType || "Strapping Machine"} onChange={e => update("machineType", e.target.value)} style={inputStyle}>{MACHINE_TYPES.map(v => <option key={v}>{v}</option>)}</select></FormField>
        <FormField label="Application"><input value={formData.application || ""} onChange={e => update("application", e.target.value)} style={inputStyle} placeholder="Customer packaging requirement" /></FormField>
        ...
        <FormField label="Installation Location"><input value={formData.installationLocation || ""} onChange={e => update("installationLocation", e.target.value)} style={inputStyle} /></FormField>
...
const CRMDashboard = ({ leads, proposals, followups, paymentReminders = [], machines = [], consumables = [], replacements = [], canViewMachines = true, navigate, userOptions = DEFAULT_USER_OPTIONS }) => {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {canViewMachines && <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div><h2 style={{ fontSize: 18, margin: 0 }}>Machines & After-Sales</h2><p style={{ margin: "4px 0 0", fontSize: 12, color: COLORS.neutral }}>Installed base, upcoming service, repeat supplies and replacement pipeline</p></div>
          <Button size="sm" variant="secondary" onClick={() => navigate("/crm/machines")}>Open Machines & Service</Button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 16 }}>
          <KPICard icon={Settings} label="Active Machines" value={activeMachines.length} color={COLORS.primary} trend={`${machines.length} installed`} onClick={() => navigate("/crm/machines")} />
          <KPICard icon={Calendar} label="Service Due / Overdue" value={serviceDueMachines.length} color={serviceDueMachines.length ? COLORS.warning : COLORS.success} trend="Within 7 days" onClick={() => navigate("/crm/machines")} />
          <KPICard icon={Repeat2} label="Consumables Due" value={consumablesDue.length} color={consumablesDue.length ? COLORS.warning : COLORS.info} trend="Within 7 days" onClick={() => navigate("/crm/machines")} />
          <KPICard icon={TrendingUp} label="Replacement Opportunities" value={openReplacements.length} color={COLORS.purple} trend={formatCurrency(replacementPipelineValue)} onClick={() => navigate("/crm/machines")} />
        </div>
      </div>}
...
const LeadsPage = ({ leads, followups = [], userOptions = DEFAULT_USER_OPTIONS, onAddLead, onImportLeads, onEditLead, onDeleteLead, onAiCall, onAddFollowup, onRefresh }) => {
  ...
  const blankForm = {
    ...
    customFields: {},
    contactPersons: [], productCategory: "Packaging Machine", machineType: "Strapping Machine", application: "",
...
const fetchAll = async () => {
  ...
  const jobs = [
    ...
    ...(isAdmin ? [
      ...
      ['Users',fetchAllUsers,setRegisteredUsers,'users'],
    ] : []),
  ];
  try {
