  ...
  const captureLiveLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        setFormData((current) => ({
          ...current,
          gpsLatitude: Number(coords.latitude.toFixed(7)),
          gpsLongitude: Number(coords.longitude.toFixed(7)),
      ...
      (error) => {
        setLocating(false);
        alert(error.code === 1 ? "Location permission was denied. Please allow location access in the browser." : `Unable to capture location: ${error.message}`);
      },
...
const fetchAll = async () => {
  ...
  try {
    const results = await Promise.allSettled([
      ...
      crmAPI.fetchCampaigns(), crmAPI.fetchContacts(), crmAPI.fetchCustomerSuccess(), crmAPI.fetchPaymentReminders(),
      crmAPI.fetchMachines(), crmAPI.fetchMachineConsumables(), crmAPI.fetchMachineReplacements(), fetchAllUsers(),
    ]);
    const [lR, pR, fR, cR, ctR, csR, payR, mR, conR, repR, uR] = results.map(r => r.status === "fulfilled" ? r.value : {});
    setLeads(lR.leads || []);
    ...
    setMachines(mR.machines || []);
    setConsumables(conR.consumables || []);
    setReplacements(repR.replacements || []);
