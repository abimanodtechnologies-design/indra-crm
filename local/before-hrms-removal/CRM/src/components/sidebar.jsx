...
import { HeartHandshake, Search, ChevronDown, LogOut, UsersRound } from "lucide-react";
...
const navItems = [
  ...
  {
    label: "HRMS", icon: UsersRound, path: "/hrms", feature: FEATURES.HRMS, adminOnly: true,
    children: [
...
function childMatches(c, pathname) {
  ...
  if (c.path === "/hrms") return pathname === "/hrms";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    crmAPI.fetchSettings().then(({ settings }) => setCrmIndustry(settings?.crm_industry || "general")).catch(() => {});
  }, []);
