...
const syncUserToHrms = async (user) => {
  if (!user?.id || !user?.email) return;
  await pool.query(`
    INSERT INTO hrms_employees
      (employee_code, name, job_title, department, email, phone, joined_on, status, monthly_salary)
    VALUES
      ($1, $2, $3, $4, $5, $6, CURRENT_DATE, $7, $8)
  ...
  `, [
    ...
    user.email,
    user.phone || null,
    String(user.status || 'active').toLowerCase() === 'active' ? 'Active' : 'Inactive',
    Number(user.basic_salary) || 0,
  ]);
};
...
const createUser = async (req, res) => {
  try {
    ...
    await syncUserToHrms(result.rows[0]);
