...
async function main(){
 ...
 try{
  ...
  for(const url of ['/crm/leads','/crm/followups','/crm/proposals','/crm/contacts','/crm/campaigns','/crm/payment-reminders','/crm/customer-success','/crm/settings','/users','/roles','/hrms/employees'])await api(url);
  ...
  console.log('PASS: login, permissions, 12 module reads, local lead persistence, follow-up/proposal/campaign writes, disabled email, backup and isolated restore.');
 }finally{
