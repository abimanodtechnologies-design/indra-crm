...
router.use(async (req, res, next) => {
  ...
  try {
    ...
    if(req.method==='GET' && req.path==='/leads/assignees')return next();
...
router.use(async (req,res,next)=>{
 ...
 const body=req.body||{};
 if(body.assigned && !req.crmIsSales){
  try{const {rows}=await pool.query("SELECT id FROM users WHERE LOWER(status)='active' AND (LOWER(email)=LOWER($1) OR LOWER(full_name)=LOWER($1) OR id::text=$1)",[String(body.assigned).trim()]);if(rows.length!==1)return res.status(400).json({error:'Select an active registered user as the assigned person.'});}
  catch{return res.status(503).json({error:'Unable to check assigned user'});}
 }
 ...
 if(/^\/leads(?:\/[^/]+)?$/.test(req.path)){
  const enquiryError = require('../services/enquiryValidation')(body.customFields?.enquiry);
  if(enquiryError)return res.status(400).json({error:enquiryError});
...
const CUSTOMER_SUCCESS_PLAN = [{stage:'Order Confirmed',days:0,title:'Verify customer order and PO',type:'Call',category:'Admin'}];
...
const PAYMENT_REMINDER_PLAN = [
  ...
  { stage: 'Advance Payment Pending', days: 2, title: 'Payment reminder 1', type: 'Email', category: 'Sales' },
  { stage: 'Balance Payment Pending', days: 5, title: 'Payment reminder 2', type: 'Call', category: 'Sales' },
  { stage: 'Overdue Payment', days: 7, title: 'Final payment reminder', type: 'Call', category: 'Sales' },
];
...
const ensurePaymentReminderTable = async () => {
  ...
  await pool.query(`ALTER TABLE crm_payment_reminders ADD COLUMN IF NOT EXISTS advance_amount NUMERIC DEFAULT 0, ADD COLUMN IF NOT EXISTS paid_amount NUMERIC DEFAULT 0, ADD COLUMN IF NOT EXISTS balance_amount NUMERIC DEFAULT 0, ADD COLUMN IF NOT EXISTS invoice_number TEXT, ADD COLUMN IF NOT EXISTS order_number TEXT, ADD COLUMN IF NOT EXISTS credit_terms TEXT, ADD COLUMN IF NOT EXISTS payment_history JSONB DEFAULT '[]'::jsonb`);
  await pool.query(`UPDATE crm_payment_reminders SET current_stage=CASE current_stage WHEN 'Reminder 1' THEN 'Advance Received' WHEN 'Reminder 2' THEN 'Balance Payment Pending' WHEN 'Final Reminder' THEN 'Overdue Payment' WHEN 'Payment Received' THEN 'Fully Paid' ELSE current_stage END WHERE current_stage IN ('Reminder 1','Reminder 2','Final Reminder','Payment Received')`);
  await pool.query(`UPDATE crm_payment_reminders SET paid_amount=CASE WHEN current_stage IN ('Payment Received','Fully Paid') THEN amount ELSE COALESCE(paid_amount,0) END, balance_amount=GREATEST(COALESCE(amount,0)-COALESCE(paid_amount,0),0) WHERE balance_amount IS NULL OR balance_amount=0`);
};
...
const ensureCustomerSuccessTable = async () => {
  ...
  await pool.query(`ALTER TABLE crm_customer_success ADD COLUMN IF NOT EXISTS order_details JSONB NOT NULL DEFAULT '{}'::jsonb`);
};
...
const resolveLeadAssignee = async lead => {
 ...
 const {rows}=await pool.query("SELECT full_name,email FROM users WHERE LOWER(status)='active' AND (LOWER(email)=LOWER($1) OR LOWER(full_name)=LOWER($1) OR id::text=$1)",[String(lead.assigned).trim()]);
 return rows.length===1 ? (rows[0].full_name || rows[0].email) : null;
};
...
const syncMachineRequirementFollowup = async (lead, requestedDate, activity) => {
  ...
  await ensureDailyDigestTable();
  const title = `Enquiry Follow-up: ${activity || 'Customer follow-up'}`;
  ...
  const existing = await pool.query(
    `SELECT id FROM crm_followups WHERE lead_id=$1 AND (title LIKE 'Machine Requirement:%' OR title LIKE 'Enquiry Follow-up:%')
  ...
  const { rows } = await pool.query(
    `INSERT INTO crm_followups
     (lead_name, title, status, type, category, assigned, assigned_user_id, start_time, end_time, description, lead_id)
     VALUES ($1,$2,'Scheduled','Call','Sales',$3,$4,$5,$6,$7,$8) RETURNING *`,
    [lead.name, title, lead.assigned || null, assignedUserId, start, end, lead.requirement_details || null, String(lead.id)]
  );
  return rows[0];
};
...
router.get('/leads/assignees', async(req,res)=>{
 try{const {rows}=await pool.query("SELECT id,full_name,email,role,status FROM users WHERE LOWER(status)='active' AND ($1::boolean OR id::text=$2) ORDER BY full_name,email",[req.crmIsAdmin,String(req.user.id)]);res.json({users:rows});}
 catch{res.status(503).json({error:'Unable to load registered users'});}
});
...
router.post('/payment-reminders/:id/send-reminder', async (req,res)=>{ try { await ensurePaymentReminderTable(); const q=await pool.query('SELECT * FROM crm_payment_reminders WHERE id=$1',[req.params.id]); const reminder=q.rows[0]; if(!reminder)return res.status(404).json({error:'Payment reminder not found'}); const sent=await sendPaymentReminderStageEmail(reminder, reminder.current_stage); res.json({success:true,sent}); } catch(err){res.status(500).json({error:err.message});} });

router.put('/payment-reminders/:id', async (req, res) => {
  try {
    await ensurePaymentReminderTable();
    const { currentStage, current_stage, status, notes, assigned, dueDate, due_date, advanceAmount, paidAmount, paymentAmount, invoiceNumber, orderNumber, creditTerms, paymentNote } = req.body;
    const requestedStage = currentStage || current_stage || PAYMENT_REMINDER_STAGES[0];
    const legacyStage = { 'Reminder 1':'Advance Received', 'Reminder 2':'Balance Payment Pending', 'Final Reminder':'Overdue Payment', 'Payment Received':'Fully Paid' };
    const stage = legacyStage[requestedStage] || requestedStage;
    if (!PAYMENT_REMINDER_STAGES.includes(stage)) return res.status(400).json({ error: 'Invalid payment status.' });
    ...
    const priorPaid = Number(existing.paid_amount || 0);
    const amountReceived = paymentAmount !== undefined && paymentAmount !== '' ? Number(paymentAmount) : null;
    if (amountReceived !== null && (!Number.isFinite(amountReceived) || amountReceived < 0)) return res.status(400).json({error:'Payment amount must be a non-negative number.'});
    const nextPaid = Math.min(total, Math.max(0, Number(paidAmount ?? priorPaid) + (amountReceived || 0)));
    const nextAdvance = Math.min(total, Math.max(0, Number(advanceAmount ?? existing.advance_amount ?? 0)));
    const computedStage = nextPaid >= total && total > 0 ? 'Fully Paid' : (nextPaid > 0 ? (nextAdvance > 0 ? 'Balance Payment Pending' : 'Advance Received') : stage);
    const stageChanged = existing.current_stage !== computedStage;
    ...
    const isPaid = computedStage === 'Fully Paid';
    const nextStatus = status || (isPaid ? 'Completed' : 'Pending');
    const paidAt = isPaid ? 'NOW()' : 'NULL';
    const paymentHistory = [...(Array.isArray(existing.payment_history) ? existing.payment_history : []), ...(amountReceived ? [{amount: amountReceived, note: paymentNote || 'Payment recorded', date: new Date().toISOString()}] : [])];

    const { rows } = await pool.query(
      `UPDATE crm_payment_reminders SET
        current_stage=$1, status=$2, notes=$3, assigned=$4, due_date=$5, advance_amount=$6, paid_amount=$7, balance_amount=$8, invoice_number=COALESCE($9,invoice_number), order_number=COALESCE($10,order_number), credit_terms=COALESCE($11,credit_terms), payment_history=$12, stage_history=$13, paid_at=${paidAt}, updated_at=NOW()
       WHERE id=$14 RETURNING *`,
      [computedStage, nextStatus, notes ?? existing.notes, assigned || existing.assigned, dueDate || due_date || existing.due_date, nextAdvance, nextPaid, Math.max(total-nextPaid,0), invoiceNumber ?? null, orderNumber ?? null, creditTerms ?? null, jsonValue(paymentHistory, []), jsonValue(nextHistory, []), req.params.id]
    );

    const reminder = rows[0];
    const paymentEmailSent = stageChanged ? await sendPaymentReminderStageEmail(reminder, computedStage) : false;
...
router.get('/customer-success', async (req, res) => {
  try {
    ...
    const {rows: orders} = await pool.query("SELECT * FROM crm_leads WHERE stage='Won' OR custom_fields->'enquiry'->>'status'='Order Received'");
    for(const lead of orders) await ensureCustomerSuccessJourney(lead);
    const {rows} = await pool.query('SELECT * FROM crm_customer_success ORDER BY updated_at DESC, id DESC');
...
router.post('/customer-success/from-lead/:leadId', async (req, res) => {
  try {
    ...
    const lead = rows[0];
    if (!lead) return res.status(404).json({ error: 'Lead not found' });
    if (lead.stage !== 'Won' && lead.custom_fields?.enquiry?.status !== 'Order Received') return res.status(400).json({error:'Receive an order before starting Customer Success.'});
...
router.put('/customer-success/:id', async (req, res) => {
  try {
    await ensureCustomerSuccessTable();
    const { currentStage, current_stage, status, notes, assigned, dueDate, due_date, orderDetails } = req.body;
    ...
    const stage = currentStage || current_stage || existing.current_stage;
    if (!CUSTOMER_SUCCESS_STAGES.includes(stage) && stage!==existing.current_stage) return res.status(400).json({error:'Invalid customer success stage'});
    if(orderDetails!==undefined && (!orderDetails || typeof orderDetails!=='object'||Array.isArray(orderDetails)))return res.status(400).json({error:'Invalid order details'});
    const details = orderDetails===undefined ? existing.order_details : {...existing.order_details,...orderDetails};
    const error = require('../services/customerSuccessValidation')(details,stage);
    ...
    const cancelled = stage === 'Lost / Cancelled Order';
    const nextStatus = cancelled ? 'Cancelled' : (stageChanged ? 'Active' : (status || existing.status || 'Active'));
    if(!['Active','Completed','Cancelled'].includes(nextStatus))return res.status(400).json({error:'Invalid journey status'});
    const completedAt = ['Completed','Cancelled'].includes(nextStatus) ? 'COALESCE(completed_at,NOW())' : 'NULL';
    ...
    ({ rows } = await client.query(
      `UPDATE crm_customer_success SET
        ...
        stage_history=$6, order_details=$8, completed_at=${completedAt}, updated_at=NOW()
       WHERE id=$7 RETURNING *`,
      [stage, nextStatus, notes ?? existing.notes, assigned || existing.assigned, dueDate || due_date || existing.due_date, jsonValue(nextHistory, []), req.params.id, JSON.stringify(details || {})]
    ));
    const title='Customer Success: Repeat order reminder #'+existing.id;
    if(cancelled || (orderDetails!==undefined && Object.prototype.hasOwnProperty.call(orderDetails,'repeatDate'))) {
      const pending=await client.query("SELECT id FROM crm_followups WHERE lead_id=$1 AND title=$2 AND status IN ('Scheduled','Pending') ORDER BY id LIMIT 1",[existing.lead_id,title]);
      if(details.repeatDate && !cancelled){
        const assignedId=await resolveAssignedUserId(rows[0].assigned);
        const start=details.repeatDate+'T09:00:00', end=details.repeatDate+'T09:30:00';
        if(pending.rowCount) await client.query('UPDATE crm_followups SET start_time=$1,end_time=$2,description=$3,assigned=$4,assigned_user_id=$5,updated_at=NOW() WHERE id=$6',[start,end,details.repeatNotes||'',rows[0].assigned,assignedId,pending.rows[0].id]);
        else await client.query("INSERT INTO crm_followups(lead_id,lead_name,title,status,type,category,assigned,assigned_user_id,start_time,end_time,description) VALUES($1,$2,$3,'Scheduled','Call','Sales',$4,$5,$6,$7,$8)",[existing.lead_id,existing.lead_name,title,rows[0].assigned,assignedId,start,end,details.repeatNotes||'']);
      } else if(pending.rowCount) await client.query("UPDATE crm_followups SET status='Cancelled',updated_at=NOW() WHERE id=$1",[pending.rows[0].id]);
    }
    await client.query('COMMIT');
