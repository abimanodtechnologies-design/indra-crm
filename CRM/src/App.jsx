...
function AppLayout() {
  const {loaded, permissionError, loadPermissions} = usePermissions();
  if (!loaded) return <main style={{padding:32}}>Loading CRM modules...</main>;
  if (permissionError) return <main style={{padding:32}}><h1>CRM connection unavailable</h1><p role="alert">{permissionError}</p><button onClick={loadPermissions}>Retry</button></main>;
  return (
